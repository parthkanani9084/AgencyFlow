...
import {
  TrendingUp, DollarSign, Users, Target, ChevronDown,
...
export default function AdsTrackingPage() {
  ...
  const totals = {
    spend:  adCampaigns.reduce((s, c) => s + c.spent, 0),
    leads:  adCampaigns.reduce((s, c) => s + c.leads, 0),
    clicks: adCampaigns.reduce((s, c) => s + c.clicks, 0),
    avgRoas: (adCampaigns.reduce((s, c) => s + c.roas, 0) / adCampaigns.length).toFixed(1),
  };

  const kpis = [
    { label: 'Total Spend',   value: `$${totals.spend.toLocaleString()}`,  icon: DollarSign, color: 'text-violet-600 bg-violet-50', change: '+12%', up: true },
    { label: 'Total Leads',   value: totals.leads.toLocaleString(),         icon: Users,      color: 'text-blue-600 bg-blue-50',    change: '+24%', up: true },
    { label: 'Total Clicks',  value: totals.clicks.toLocaleString(),        icon: TrendingUp, color: 'text-emerald-600 bg-emerald-50', change: '+18%', up: true },
    { label: 'Avg ROAS',      value: `${totals.avgRoas}×`,                  icon: Target,     color: 'text-amber-600 bg-amber-50',  change: '-0.3×', up: false },
  ];
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
            ...
            <div className="space-y-3">
              {platformBreakdown.map((p) => {
                const pct = Math.round((p.spend / totals.spend) * 100);
                return (
                  <div key={p.platform}>
