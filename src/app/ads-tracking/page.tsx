...
export default function AdsTrackingPage() {
  ...
  const filtered = adCampaigns.filter(
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
              <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                ...
                <button onClick={() => setIsAdsModalOpen(false)} className="text-slate-400 hover:text-slate-600">
                  <X size={20} />
                </button>
              </div>
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[12.5px] font-medium text-slate-700 mb-1.5">Add to Spend (₹)</label>
                    <input
                      type="number"
                      className="w-full text-[13px] border border-slate-200 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-violet-500/30"
                      placeholder="0"
                      onChange={(e) => setAdsForm({ ...adsForm, spend: Number(e.target.value) })}
                    />
                  </div>
                  <div>
                    <label className="block text-[12.5px] font-medium text-slate-700 mb-1.5">Add to Leads</label>
                    <input
                      type="number"
                      className="w-full text-[13px] border border-slate-200 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-violet-500/30"
                      placeholder="0"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[12.5px] font-medium text-slate-700 mb-1.5">Add to Clicks</label>
                    <input
                      type="number"
                      className="w-full text-[13px] border border-slate-200 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-violet-500/30"
                      placeholder="0"
                  ...
                  <div>
                    <label className="block text-[12.5px] font-medium text-slate-700 mb-1.5">Average ROAS (×)</label>
                    <input
                      type="number"
                      step="0.1"
                      className="w-full text-[13px] border border-slate-200 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-violet-500/30"
                ...
                <div className="pt-4 flex justify-end gap-2 border-t border-slate-100">
                  ...
                  <button
                    onClick={() => {
                      const newSpend = adsMetrics.totalSpend + (adsForm.spend || 0);
                      const newLeads = adsMetrics.totalLeads + (adsForm.leads || 0);
                      const newClicks = adsMetrics.totalClicks + (adsForm.clicks || 0);
                      const newRoas = adsForm.roas || adsMetrics.avgRoas;
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
            ...
            <div className="space-y-3">
              {platformBreakdown.map((p) => {
                const platformTotalSpend = platformBreakdown.reduce((sum, item) => sum + item.spend, 0);
                const pct = Math.round((p.spend / platformTotalSpend) * 100);
                return (
                  <div key={p.platform}>
                  ...
                  return (
                    ...
                    >
                      <td className="px-5 py-3.5">
                        ...
                        <p className="text-[11px] text-slate-400">{c.client}</p>
                      </td>
                      <td className="px-5 py-3.5 hidden md:table-cell">
                        <span className={`inline-flex px-2.5 py-0.5 rounded-full text-[11px] font-semibold ${platformColors[c.platform]}`}>
                          {c.platform}
                        </span>
                      </td>
                      <td className="px-5 py-3.5 text-right">
                        <p className="font-semibold text-slate-800 tabular-nums">₹{c.spent.toLocaleString()}</p>
                      ...
                      <td className="px-5 py-3.5 text-right text-slate-700 font-semibold tabular-nums hidden sm:table-cell">{c.leads}</td>
                      <td className="px-5 py-3.5 text-right text-slate-600 tabular-nums hidden lg:table-cell">{c.clicks.toLocaleString()}</td>
                      <td className="px-5 py-3.5 text-right">
                        <span className={`font-bold tabular-nums ${c.roas >= 4 ? 'text-emerald-600' : c.roas >= 3 ? 'text-amber-600' : 'text-red-500'}`}>
                          {c.roas}×
                        </span>
                      </td>
                      <td className="px-5 py-3.5">
                        <span className={`inline-flex px-2.5 py-0.5 rounded-full text-[11px] font-semibold capitalize ${statusBadge[c.status]}`}>
                          {c.status}
                        </span>
                      </td>
                    </tr>
